...
#define kItemWidth [UIScreen mainScreen].bounds.size.width * 0.8     // item宽高
static CGFloat const kPaddingMid = 15.f;    // item间距
static CGFloat const kPaddingLeft = 20.f;   // 最左边item左边距


@interface RecommendCollectionFlowLayout()<UIScrollViewDelegate, UICollectionViewDelegate> {
...
- (void)prepareLayout
{
    ...
    self.itemSize = CGSizeMake(kItemWidth, 85);
...
//    self.sectionInset = UIEdgeInsetsMake(paddingVertical, kPaddingLeft, paddingVertical, paddingRight);
        // mdf by caf
    self.sectionInset = UIEdgeInsetsMake(0, kPaddingLeft, paddingVertical+16, paddingRight);
}
